...
smazVV p [] = []
smazVV p (h:t) = if (p==h) then smazVV p t else h:(smazVV p t)

smazVZC [] = []
smazVZC (h:t) = if(h>0) then h:(smazVZC t) else smazVZC t

smazVMSM m [] = []
smazVMSM m (h:t) = if ((mod m 2)==0) then smazVMSM (m+1) t else h:(smazVMSM (m+1) t )
...
vSC = [(x) |  x<-[2,4..500], ((mod x 3)==0) && ((mod x 7)==0)]